...
def ensure_venv():
    ...
    if not venv_dir.exists():
        ...
        result = subprocess.run([sys.executable, "-X", "utf8", str(setup_script)])
...
def main():
    ...
    # Build command (use -X utf8 on Windows to avoid cp1252 encoding errors with emoji)
    cmd = [str(venv_python), "-X", "utf8", str(script_path)] + script_args
